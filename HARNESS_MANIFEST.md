...
*which bets are due for a re-check* something you can **read** instead of remember. It sorts parts by
**shelf-life** — how each one ages: **permanent** parts never expire (their force is a property of the
world, not the model), **depreciating** parts get re-audited at every model or tool upgrade (they exist
only because a model/tool once needed them), **appreciating** parts grow more valuable as the model
improves.
...
- **Model upgrade** — re-run your behavioral evals (`scripts/eval.sh`) to prove the new model helped,
